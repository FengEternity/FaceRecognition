...
def main():
    ...
    if not (args.extract or args.evaluate):  # train + val
        ...
        train_longest_size = max(
            [int(np.ceil(len(td) / float(bs))) for td, bs in zip(train_dataset, args.train.batch_size)])
        train_sampler = [GivenSizeSampler(td, total_size=train_longest_size * bs, rand_seed=args.train.rand_seed) for
                         td, bs in zip(train_dataset, args.train.batch_size)]
        ...
        if args.val.flag:
            ...
            val_longest_size = max(
                [int(np.ceil(len(vd) / float(bs))) for vd, bs in zip(val_dataset, args.val.batch_size)])
            val_sampler = [GivenSizeSampler(vd, total_size=val_longest_size * bs, sequential=True) for vd, bs in
                           zip(val_dataset, args.val.batch_size)]
    ...
    if args.test.flag or args.evaluate:  # online or offline evaluate
        ...
        test_sampler = [GivenSizeSampler(td,
                                         total_size=int(
                                             np.ceil(len(td) / float(args.test.batch_size)) * args.test.batch_size),
    ...
    if args.extract:  # feature extraction
        ...
        extract_sampler = GivenSizeSampler(
            extract_dataset, total_size=int(
                np.ceil(len(extract_dataset) / float(args.extract_info.batch_size)) * args.extract_info.batch_size),
            sequential=True)
    ...
    model = models.MultiTaskWithLoss(backbone=args.model.backbone, num_classes=args.num_classes,
    ...
    ## feature extraction
    if args.extract:
        extract(extract_loader, model, num=len(extract_dataset),
                output_file="{}_{}.bin".format(args.load_path[:-8], args.extract_info.data_name))
        return

    ######################## train #################
    ## lr scheduler
    lr_scheduler = torch.optim.lr_scheduler.MultiStepLR(optimizer, args.train.lr_decay_steps,
                                                        gamma=args.train.lr_decay_scale, last_epoch=start_epoch - 1)

    ## logger
    logging.basicConfig(
        filename=os.path.join('{}/logs'.format(args.save_path), 'log-{}-{:02d}-{:02d}_{:02d}:{:02d}:{:02d}.txt'.format(
